...
## TLDR
- The skill interviews you about personas, technologies, projects, funding, and first-party signals.
- It calibrates against your won deals, then gives you sliders to tune the score by hand.
- Every signal deep-links back into Sumble, so reps can move from rank to people, teams, jobs, and talking points.
...
- **Score everything, or a sample — your call.** For a small or mid-size list, score it all. For a large CRM — where most accounts are stale and scoring all of them just burns credits — narrow it: to the rep-allocated accounts, a subset you specify, or a **stratified sample** the skill draws for you (≈30% closed-won, 40% rep-allocated, 30% unallocated) that oversamples the accounts that matter and still gives the evaluation a representative mix. It shows the credit estimate before any large pull.
...
Funding recency uses an inverted variant of the same normaliser (`recency` transform: `norm = clamp(1 − log1p(days) / log1p(p99), 0, 1)`) so that fewer days since the last round → a higher score, and never-financed → 0.
