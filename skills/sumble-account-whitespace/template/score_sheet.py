...
weights config, and writes ONE file:
  - score.csv : the human-facing scored sheet, sorted by rank. Columns:
                `rank` (far left) → identity incl. `sumble_url` (the company's
                Sumble page, next to its own url) → `score` → one CONTRIBUTION
                column per signal (points = norm * effective_weight * multiplier
                * 100, so the contributions SUM EXACTLY to `score`) → one per-signal
                deep link on the far right. Signals whose TOTAL contribution across
                all rows is 0 (e.g. a category/within weight of 0) are dropped.
...
def build_score_sheet(app_dir: Path, weights_path: Path | None = None) -> dict[str, Any]:
    ...
    # The company's Sumble page link sits in the identity block, right after the
    # company's own `url` (or after `name`). Sentinel "sumble_url" → org link.
    left: list[str] = []
    for c in ident:
        left.append(c)
        if c == "url":
            left.append("sumble_url")
    if "sumble_url" not in left:
        pos = left.index("name") + 1 if "name" in left else len(left)
        left.insert(pos, "sumble_url")

    # Layout: rank (far left) → identity (incl. company sumble_url) → score →
    # contribution cols → per-signal deep links (far right). Contribs sum to score.
    score_fields = (
        ["rank"] + left + ["score"]
        + [labels[k] for k in live]
        + [f"{labels[k]} link" for k in link_keys]
    )
    ...
    with tmp2.open("w", newline="", encoding="utf-8") as f:
        ...
        for i in order:
            ...
            org_url = f"{base_url}{slug}" if slug else ""
            left_vals = [org_url if c == "sumble_url" else rows[i].get(c, "") for c in left]
            row = [rank_of[i]] + left_vals + [scores[i]]
