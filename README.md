...
| [`sumble-account-research`](skills/sumble-account-research) | Two modes, picked in one question up front: **deep dive on a specific account**, or **help me prioritize across my accounts**. Then it runs to a finished deliverable without interrupting you — loading your company profile, pulling internal context (call notes, pipeline stage, existing business, goals), rebuilding the account's Sumble overview (tech, teams, people, headcount, hiring signals, ICP fit), and recommending the team to land or expand into plus the buying group to reach. Delivers outreach sequences, an account plan, a deck, or call prep. | — |
...
Each skill ships a real, runnable example under `examples/`.

**▶ Account scoring — live demo: https://account-scoring-demo.sumble.com/**

[`examples/account-scoring`](examples/account-scoring)
...
[`examples/crm-cleaning`](examples/crm-cleaning) is the
...
The territory-planning method is written up in
